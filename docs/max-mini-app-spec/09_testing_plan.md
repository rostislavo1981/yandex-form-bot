...
- Реальная MAX Bot API, реальные токены, реальный webhook delivery.
- Реальный VPS и HTTPS-сертификат.
- UI на реальном мобильном устройстве.

Эти пункты проходятся только ручной приёмкой с реальными credentials.
