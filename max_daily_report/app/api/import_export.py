...
async def _read_catalog_workbook(file: UploadFile) -> bytes:
    if not file.filename or not file.filename.lower().endswith(".xlsx"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Требуется файл .xlsx",
        )
    content = await file.read(settings.max_catalog_upload_bytes + 1)
    if len(content) > settings.max_catalog_upload_bytes:
        raise HTTPException(
            status_code=status.HTTP_413_CONTENT_TOO_LARGE,
            detail="Файл справочников слишком большой",
        )
    return content
...
async def validate_catalog_import(
    ...
    content = await _read_catalog_workbook(file)
